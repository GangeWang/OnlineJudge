...
登入時，系統會記錄連線 IP 與瀏覽器保存的裝置識別碼；同一帳號在三小時內由不同裝置識別碼登入時，後端會記錄並輸出警告，兩個已登入的瀏覽器都會顯示異地登入提示（另一台瀏覽器最晚於 30 秒內更新）。IP 會保留在警告中供比對，但不會只因 IP 改變而把同一台裝置誤判為異地登入。
...
同一瀏覽器按 F5 時，`localStorage` 內的裝置識別碼不會改變；後端也會忽略三小時內已看過的相同裝置識別碼，因此重新整理不會被判定為異地登入。
* 題目列表
* 提交程式
* 判題結果
* 資料庫紀錄
